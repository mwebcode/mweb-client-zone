...
const ServiceAccountDetails: React.FC<ServiceAccountDetailsProps> = ({ account, onBack }) => {
  ...
  const options = {
    ...
    plugins: {
      legend: {
        position: "top" as "top", // Ensure this is one of the valid values: "top", "bottom", "left", "right", etc.
      },
    },
  };
  ...
  return (
    <div className="container">
      <button className="back-button" onClick={onBack}>Back to My Service Accounts</button>
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
    {isEditing ? (
      <input
        ...
        style={{ width: '200px', marginRight: '8px', fontSize: '18px', fontWeight: 'bold', color: '#004D6E' }}
      />
    ) : (
      <>
        <span style={{ fontSize: '24px', fontWeight: 'bold', color: '#004D6E' }}>{friendlyName}</span>
        <img 
    ...
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent>
          <p>Struggling to keep track of all of your products?
            You can assign a friendly name to each  o your Mweb
            products, e.g. "Business Website", or "Mom's Fibre"
            and they'll appear like that everywhere - including your statements.
          </p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  ...
  <span className="account-price">R{account.contracts[0]?.activeRate} PM</span>
  </h2>
      </div>
  ...
  <tr>
    ...
    <td>
      ...
      {service.hasServiceSettings && !service.hasServiceActions && (
        <FontAwesomeIcon
          icon={faGear}
          size="lg"
          onClick={() => {
          ...
          style={{ cursor: 'pointer', color: '#000', marginRight: '8px' }}
        />
      )}
  ...
  {expandedServiceId === service.id && (
    <tr>
      ...
      <button
        className={`option-button ${selectedOption === "30 Days" ? "active" : ""}`}
        onClick={() => handleClick("30 Days")}
      >
        30 Days
      </button>
      <button
        className={`option-button ${selectedOption === "12 Months" ? "active" : ""}`}
    ...
    <div className="chart-container">
        <Bar data={data1} options={options} />
          ...
          <tr>
            <td colSpan={4} className="text-center">No included services available.</td>
          </tr>
        )}
      </tbody>
    </table>
  ...
  <DialogTrigger asChild>
    <button onClick={() => setIsDialogOpen(true)} style={{ display: 'none' }}>Open Dialog</button>
  </DialogTrigger>

  <DialogContent className="dialog-content">
    <div className="modal-settings">
      <h3>{serviceData?.dialogTitle} Settings - {serviceData?.username}</h3>
      ...
      <Accordion type="single" collapsible>
        <AccordionItem value="item-2">
          <AccordionTrigger>{groups && groups.length > 0 ? groups[0].displayGroup : 'Default Group Name'}</AccordionTrigger>
          <AccordionContent>
          <div className="router-credentials">
      <h4>Username: {serviceData?.username+"@mwb.com"}</h4>
      <p>
        The username to enter into your Fibre router. Note: this may be different from your email address.
      </p>
      ...
      <div className="password-field">
        <input
          ...
          id="password"
          placeholder="********"
          value={groups && groups.length > 0 ? groups[0].settings[0].name : ''}
          readOnly
        />
      ...
      <p>{groups && groups.length > 0 ? groups[0].settings[0].description :'Password to enter into your Fibre router.'}</p>
    </div>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
      ...
      <Accordion type="single" collapsible>
        <AccordionItem value="item-3">
          <AccordionTrigger>{groups && groups.length > 0 ? groups[1].displayGroup : 'Default Group Name'}</AccordionTrigger>
          <AccordionContent>
          <div className="form-container">
      <div className="form-group">
        <label htmlFor="mobileNumber">On-site Contact Mobile Number</label>
        <input
          type="text"
          id="mobileNumber"
        ...
        />
        <small>{mobileNumberSetting?.description}</small>
      </div>

      <div className="form-group">
        <label htmlFor="email">On-Site Contact Email Address</label>
        <input
          type="email"
          id="email"
      ...
      <div className="form-group">
        <label htmlFor="telephoneNumber">On-Site Contact Telephone Number</label>
        <input
          type="text"
          id="telephoneNumber"
        ...
        />
        <small>{telephoneSetting?.description}</small>
      </div>
      ...
      <div className="form-group">
        ...
        <select
          id="propertyOwner"
          value={propertyOwnerSetting?.value === 1 ? 'yes' : 'no'}
          onChange={(e) => handleInputChange(e, 'propertyOwner')}
        >
          <option value="yes">Yes</option>
          <option value="no">No</option>
        </select>
        <small>{propertyOwnerSetting?.description}</small>
      </div>
      ...
      <Label htmlFor="option-one">{secondLineSetting?.description}</Label>
      </div>
      </RadioGroup>
      </div>
    ...
    <Button variant="outline">Save</Button>
      ...
      <Dialog open={isDialogOpen1} onOpenChange={setIsDialogOpen1}>
        ...
        <DialogContent style={{ backgroundColor: 'white' }}>
          <DialogHeader>
            ...
            <DialogDescription>
              Active your free mailbox as it has 
              not been created or was removed due to non-use
            </DialogDescription>
            <Button variant="outline">Active mailbox</Button>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={isDialogOpenAction} onOpenChange={setIsDialogOpenAction}>
        <DialogContent style={{ backgroundColor: 'white' }}>
          <DialogHeader>
            <DialogTitle>{data?.dialogTitle} Settings - {data?.serviceIdentifier}</DialogTitle>
            <DialogDescription>
            <Accordion type="single" collapsible>
            <AccordionItem value="item-1">
            <AccordionTrigger>Information</AccordionTrigger>
              ...
              {data?.settings.map(setting => (
                <div key={setting.serviceTypeParameterId}>
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      console.log(`New value for ${setting.name}: ${e.target.value}`);
                    }} 
                  />
                </div>
              ))}
